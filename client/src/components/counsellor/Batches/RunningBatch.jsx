...
const RunningBatch = () => {
  ...
  useEffect(() => {
    // Logic remains the same: dispatch action to fetch 'Running' batches
  ...
  const tableRows = useMemo(() => {
    return batches.map((batch, index) => (
      ...
      >
        ...
        {/* Status Badge (Blue/Indigo for Running) */}
        <td className="px-6 py-4 whitespace-nowrap">
          <span className="inline-flex items-center px-3 py-1 text-xs font-bold leading-5 rounded-full bg-blue-100 text-blue-800 border border-blue-300">
  ...
  if (error) {
    return (
      <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-md mx-6 my-4 shadow-sm" role="alert">
        ...
        <p>Could not load Running batches. Please check the network or try again. Details: **{error}**</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      ...
      <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-6 border-b-4 border-indigo-600 pb-2 inline-block">
        <span className="mr-3 text-indigo-600">🚀</span> Running Batches
      </h1>
      ...
      {batches.length === 0 ? (
        ...
        <div className="text-center bg-white p-16 rounded-xl shadow-lg border border-gray-200 mt-8">
          ...
          <p className="mt-2 text-gray-500">There are currently no active batches in progress.</p>
        </div>
      ) : (
