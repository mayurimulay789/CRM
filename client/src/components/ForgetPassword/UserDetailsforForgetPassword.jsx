...
import { toast } from 'react-hot-toast'; // Toaster removed, import only toast
...
const UserDetailsforForgetPassword = () => {
  ...
  const handleContinue = async () => {
    ...
    if (emailError) {
      ...
      toast.error(emailError, { id: 'email-validation' });
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${baseurl}api/auth/searchuserbyemailandreset`,
      ...
      if (response.data.success) {
        if (response.data.otpSent) {
          ...
          toast.success(response.data.message, { id: 'no-otp-sent' });
          navigate('/otpvalidationpage', {
            state: {
              email: email,
              otpSent: false,
      ...
      if (error.response) {
        ...
        if (error.response.status === 400) {
          ...
          if (error.response.data?.emailVerified === false) {
            toast.error('Please verify your email first', { id: 'email-not-verified' });
          }
          ...
          toast.error(errorMessage, { id: 'http-error' });
        }
        setApiResponse(error.response.data);
      } else if (error.request) {
        toast.error('Network error. Please check your connection.', { id: 'network-error' });
      } else {
        toast.error('An error occurred. Please try again.', { id: 'unknown-error' });
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gray-50 px-4">
      {/* Toast Notifications – REMOVED from here; place <Toaster /> in App.jsx once */}
